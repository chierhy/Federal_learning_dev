...
#print(weight)
class cnn2d_xiao_created(nn.Module):
    def __init__(self):
        super().__init__()
    ...
    def forward(self, x):
        # print(x.shape)
        x = F.conv2d(x, weights.weights1(), bias=None, stride=1, padding=2, dilation=1, groups=1)
        x = F.max_pool2d(F.relu(x),kernel_size=2, stride=2)
        # print(x.shape)
        x = F.conv2d(x, weights.weights2(), bias=None, stride=1, padding=1, dilation=1, groups=1)
        x = F.max_pool2d(F.relu(x),kernel_size=2, stride=2)
        # print(x.shape)
        x = F.conv2d(x, weights.weights3(), bias=None, stride=1, padding=1, dilation=1, groups=1)
        x = F.max_pool2d(F.relu(x),kernel_size=2, stride=2)
        # print(x.shape)
        x = F.conv2d(x, weights.weights4(), bias=None, stride=1, padding=1, dilation=1, groups=1)
        x = F.max_pool2d(F.relu(x),kernel_size=2, stride=2)
        # print(x.shape)
        x = x.view(x.size(0), -1)
        # print(x.shape)
...
cifar = xdr.FlameSet('insert_fault', 2304, '2D', 'no_base')
...
net = cnn2d_xiao_created()     # 加载训练过的模型
